...
RSpec.describe 'Answers Votes', type: :request do
  let(:answer) { create(:answer, question_id: question.id, author_id: user.id) }
  let(:question) { create(:question, author_id: user.id) }
  let(:user) { create(:user) }

  describe 'POST /answer/:id/vote' do
    context 'registered user not author' do
      before { sign_in user }

      it 'likes the question' do
        expect { post vote_answer_path(answer), params: { value: 1 } }.to change { answer.votes.where(value: 1, user: user).count }.by(1)
      ...
      it 'pressing again removes the vote' do
        post vote_answer_path(answer), params: { value: 1 }
        expect { post vote_answer_path(answer), params: { value: 1 } }.to change { answer.votes.where(value: 1, user: user).count }.by(-1)
      ...
      it 'changes the voice from like to dislike' do
        expect {
          ...
          post vote_answer_path(answer), params: { value: -1 }
          question.reload
        }.to change { answer.votes.where(value: -1, user: user).count }.by(1)
